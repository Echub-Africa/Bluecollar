...
const SignUpArtisan = () => {
  ...
  const handleSubmit = async () => {
    const payload = {
      ...formData,
      type: "artisan", // required by backend
    };

    try {
      const res = await fetch(
        "https://blucolar-be.onrender.com/api/auth/signup",
        {
          method: "POST",
          headers: {
      ...
      localStorage.setItem("artisanEmail", formData.email);
      localStorage.setItem("artisanFirstName", formData.firstName);
      localStorage.setItem("artisanLastName", formData.lastName);
