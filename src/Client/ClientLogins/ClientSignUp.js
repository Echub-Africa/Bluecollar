...
const SignUpClient = () => {
  ...
  const handleSubmit = async () => {
    const payload = {
      ...formData,
      type: "home-owner", // required by backend
    };

    try {
      const res = await fetch(
        "https://blucolar-be.onrender.com/api/client/signup",
        {
          method: "POST",
          headers: {
      ...
      localStorage.setItem("home-ownerToken", data.token);
      localStorage.setItem("home-ownerEmail", formData.email);
      localStorage.setItem("home-ownerFirstName", formData.firstName);
      localStorage.setItem("home-ownerLastName", formData.lastName);

      navigate("/clientAuth/confirmEmail");
    } catch (err) {
      ...
      <div className="all-sign">
        <div className="upper-sign">
          ...
          <div className="sign-5">
            ...
            <Link to="/clientAuth/login" className="login-btn">
              Login
              <Icon
                className="icon"
                width="14px"
