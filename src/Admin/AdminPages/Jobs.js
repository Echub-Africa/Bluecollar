...
const handleSubmit = async (e) => {
  ...
  if (!formData.artisanEmail || !formData.jobId) {
  ...
  try {
    const response = await fetch(
      "https://blucolar-be.onrender.com/api/client/project/assign-artisan",
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          artisanEmail: formData.artisanEmail,
    ...
    if (response.ok) {
      setSuccessMessage("✅ Artisan assigned successfully!");
      console.log("✅ Artisan assigned:", result);
  ...
  useEffect(() => {
    ...
    const fetchRecentData = async () => {
      try {
        const response = await fetch(
          "https://blucolar-be.onrender.com/api/admin/recent-activity",
          {
            method: "GET",
            headers: {
              "Content-Type": "application/json",
  ...
  recentJobs.forEach((job) => console.log("Job ID:", job.id));
...
const filteredJobs = recentJobs?.filter((item) =>
      ...
      <div className="containary">
        <div className="request">
          <div className="request-1">
            <h2>Jobs </h2>
            <button onClick={handleDropdown}>Assign Job</button>
          </div>
...
<form onSubmit={handleSubmit}>
  <div className="drop-body">
    <label htmlFor="artisanEmail">Artisan Email</label>
    <input
      id="artisanEmail"
      type="email"
      name="artisanEmail"
